...
//
//
// An implementation of Young, Tarjan, and Orlin's minimum mean cycle
// algorithm.
//
// @article{YoTaOr91,
...
#else
    ninfo_yto *more_ninfo;
    float     *edge_key;
    try {
        more_ninfo = new ninfo_yto[ n ];
        edge_key = new float[ m ];
...
#if 0
                        // it seems that these cases are already covered by the
                        // above case. this has to be proved.
                        else if ( e == ptr_y->key ) {
                            printf("found case1\n");
                            ptr_y->ekey = edge_key[ e ];
                            pq.update_node( ptr_y->ekey, ptr_y->key, ptr_y->node2heap );
                        } else {
                            int y = g->ith_target_node( x, i );
                            for ( int j = 0; j < g->indegree( y ); ++j ) {
                                int e2 = g->ith_source_edge( y, j );
                                if ( edge_key[ e2 ] < ptr_y->ekey ) {
                                    printf("found case2\n");
                                }
                            }
                        }
