...
//
//
// An implementation of Howard's minimum mean cycle algorithm. This
                ...
                for ( int i = 0; i < g->indegree( v ); ++i ) {
                    ...
                    if ( -1 != more_ninfo[ u ].visited ) {
                        if ( v == more_ninfo[ u ].target ) {
                            ...
                            more_ninfo[ u ].dist = more_ninfo[ v ].dist - lambda + more_ninfo[ u ].einfo;
                            nodeq.put( u );
#ifdef MAKE_POLICY_CONNECTED
#ifdef DEBUG
                            ++nvisited;
