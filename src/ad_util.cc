...
parse_args( int argc, char *argv[], args_t& args )
{
    ...
    // [-d 0/1/2] [-p n m] [-w w1 w2] [-s seed] [-f dump_file]
    //
    // Note [-t t1 t2] makes sense only for the cycle ratio version
    ...
    args.w1 = 1;
    args.w2 = 300;
    args.t1 = 1; // Used in the cycle ratio version
    args.t2 = 1; // Used in the cycle ratio version
    args.seed = -1;
    ...
    while ( i < argc ) {
        ...
        if ( !strcmp( argv[ i ], "-m" ) ) {
            ...
            if ( 2 >= ( argc - i ) ) 
                error_found = true;
            args.t1 = atoi( argv[ i + 1 ] ); // Read but ignore it
            args.t2 = atoi( argv[ i + 2 ] ); // Read but ignore it
            printf( "WARNING: '-t' is an option only for the cycle ratio version.\n" );
            args.t1 = 1; // Any other value is impossible for the cycle mean version.
            args.t2 = 1; // Any other value is impossible for the cycle mean version.
            i += 3;
